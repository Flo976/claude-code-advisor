...
Send ONE message with 5 parallel `Agent` calls (one per reference file below).
Each agent works independently — if one fails, the others still land.

Skip `skills-catalog.md` (auto-generated by the Python script in step 1) and
`modes-and-patterns.md` (the core 6-level framework, edit manually when it
evolves rather than via automated research).

Files to dispatch agents for:
- `capabilities.md`
- `mcps-catalog.md`
- `community-tips.md`
- `context-window.md`
...
After all subagents report back, append a dated section to
`~/.claude/skills/claude-code-advisor/CHANGELOG.md`:
